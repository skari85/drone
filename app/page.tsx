...
export default function Home() {
  ...
  return <main className="instrument-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark" aria-hidden="true"><span /><span /><span /></div><div><h1>HI DRONE</h1><p>synth soul / organismic composition station</p></div></div><div className="top-actions"><div className={`organism-state ${organismLinked ? 'is-linked' : ''}`} title={organism.signal}><span aria-hidden="true" />{organismLinked ? 'synth soul' : 'manual mode'}<small>{Math.round(organism.energy * organism.coherence)}%</small></div><button className={`organism-toggle ${organismLinked ? 'is-on' : ''}`} type="button" onClick={() => { setOrganismLinked((previous) => !previous); setNotice(organismLinked ? 'organism link loosened — manual currents only' : 'organism link restored — everything can feed everything'); }}><span aria-hidden="true">⌁</span> LINK</button><button className="deck-toggle" type="button" onClick={() => { const shouldOpen = !Object.values(openModules).some(Boolean); setOpenModules({ sound: shouldOpen, sequence: shouldOpen, licks: shouldOpen, behaviour: shouldOpen, patch: shouldOpen }); }}>{Object.values(openModules).some(Boolean) ? 'FOLD DECK' : 'OPEN DECK'}</button><div className={`engine-state ${powered ? 'awake' : ''}`}><span aria-hidden="true" />{powered ? 'engine awake' : 'engine asleep'}</div><button className={`power-button ${powered ? 'is-on' : ''}`} type="button" onClick={togglePower}><span aria-hidden="true">◉</span> POWER</button></div></header>
